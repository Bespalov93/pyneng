...
def write_last_log_to_csv(source_log, output):
    # Словарь для хранения самой последней записи для каждого email
    latest_records = {}

    # Чтение данных из исходного файла
    with open(source_log, 'r', newline='', encoding='utf-8') as csvfile:
        reader = csv.DictReader(csvfile)
        
        # Обрабатываем каждую строку
        for row in reader:
            email = row['Email']
            current_datetime = convert_str_to_datetime(row['Last Changed'])
            
            # Если email уже существует в latest_records, проверяем дату
            if email in latest_records:
                # Сравниваем даты и обновляем запись, если текущая дата более поздняя
                existing_datetime = convert_str_to_datetime(latest_records[email]['Last Changed'])
                if current_datetime > existing_datetime:
                    latest_records[email] = row
            else:
                # Если email еще нет в словаре, добавляем запись
                latest_records[email] = row
    ...
    with open(output, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        # Записываем только самые свежие записи для каждого пользователя

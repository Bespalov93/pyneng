...
def transform_topology(yaml_filename):
    # Открываем и загружаем топологию из YAML файла
    with open(yaml_filename) as file:
        topology = yaml.safe_load(file)

    # Преобразование топологии в формат подходящий для draw_topology
    transformed_topology = {}

    for device, interfaces in topology.items():
        for local_int, neighbors in interfaces.items():
            for neighbor, neighbor_int in neighbors.items():
                # Создаем пары кортежей для соединения
                link = (device, local_int)
                reverse_link = (neighbor, neighbor_int)

                # Проверка на дублирование (включаем только одно направление)
                if reverse_link not in transformed_topology:
                    transformed_topology[link] = reverse_link

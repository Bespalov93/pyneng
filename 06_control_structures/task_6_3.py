...
for intf, vlan_info in trunk.items():
    ...
    for command in trunk_template:
        if command.endswith("allowed vlan"):
            # Получаем ключевое слово для команды (заменяем 'del' на 'remove')
            action = "remove" if vlan_info[0] == "del" else vlan_info[0]
            vlans = ",".join(vlan_info[1:])  # Список VLAN-ов через запятую
            # Формируем и выводим команду с VLAN-ами
            print(f" {command} {action} {vlans}" if action != "only" else f" {command} {vlans}")
        else:
            print(f" {command}")

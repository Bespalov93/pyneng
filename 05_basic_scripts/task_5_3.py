...
# Шаблоны конфигураций для разных режимов
access_template = [
    "switchport mode access",
...
# Словарь для выбора шаблона на основе введенного режима
templates = {
...
mode = input("Введите режим работы интерфейса (access/trunk): ").strip().lower()  # Режим работы интерфейса
interface = input("Введите тип и номер интерфейса: ").strip()  # Номер интерфейса
vlan_info = input("Введите номер влан(ов): ").strip()  # Влан для access или список для trunk

# Получаем шаблон из словаря на основе выбранного режима
selected_template = templates.get(mode)

# Если шаблон найден, подставляем VLAN и выводим конфигурацию
